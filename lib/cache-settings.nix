...
let
  ...
  determinateNixBaseSettings = {
    # For Determinate Nix systems, custom settings go to /etc/nix/nix.custom.conf
    nix.enable = false;
  };
  ...
  darwinDeterminateNixSettings = baseSettings // {
    # On Darwin with Determinate Nix, nix.settings must not be set when nix.enable is false
  ...
  nixosDeterminateNixSettings = baseSettings // {
    # On NixOS with Determinate Nix, nix.settings must be set even when nix.enable is false
