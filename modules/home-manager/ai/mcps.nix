...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    #
    # home.sessionVariables only lands in hm-session-vars.sh, which is POSIX
    # and therefore invisible to fish (programs.fish.enable is false here, so
    # HM generates no fish translation). Emit a fish conf.d snippet from the
    # SAME attrset -- same pattern as ai/lmstudio.nix and console/cargo.nix.
    home.sessionVariables = lib.mkIf cfg.servers.zvec-grep.enable zvecGrepEnv;
    ...
    home.file = lib.mkMerge [
      # fish can't source hm-session-vars.sh (POSIX; programs.fish.enable is
      # false here so HM emits no fish translation). Generate the same vars in
      # fish syntax from the SAME attrset -- pattern matches ai/lmstudio.nix.
      (lib.mkIf cfg.servers.zvec-grep.enable {
        ".config/fish/conf.d/zvec-grep.fish".text =
          "# Generated by modules/home-manager/ai/mcps.nix -- do not edit.\n"
          + lib.concatStrings (lib.mapAttrsToList
            (k: v: "set -gx ${k} \"${toString v}\"\n")
            zvecGrepEnv);
      })
