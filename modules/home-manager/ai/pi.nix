...
in
{
  options.programs.ai.pi = {
    ...
    defaultModel = mkOption {
      ...
      # to bedrock/converse/us.anthropic.claude-opus-5-5 on the proxy side
      # with output_config.effort=xhigh (highest thinking). Pi's defaultModel is
      # provider-scoped (no `provider:` prefix), so just "claude-opus-5-5".
      #
